...
Lin (yhlin@iastate.edu) \tom{it.corresponding returns nothing} }
\affil[*]{corresponding author(s): Heike
Hofmann (hhofmann4@unl.edu) \tom{it.corresponding returns nothing} }
...
\hh{\bf overview of the study design: create dataset, manual alignment}

\hh{\bf overview of files created (table?):}

\begin{itemize}
\item
  x3p files from scans: format, explainations, link to sections
\item
...
conviction of a defendant. However, there is a lack of a standardized
method to analyze it except for visual comparison.
\tom{...literature review and motivation...} Here, we provide a data set
of
\tom{a table here gt package (https://posit.co/blog/great-looking-tables-gt-0-2/), 3 columns: folder name, explaination, link to section}

\begin{enumerate}
  \item
  120 scans of aluminum wire cut in 120 \texttt{x3p} files,
  with names like \texttt{T1AW-LI-R1.x3p},
  \tom{no name here} raw
  
  \item
  the metadata of the scans in 1 CSV file,
  with name \texttt{meta.csv}, raw
  
  \item
  corresponding profiles extracted from the scans in 120 CSV file,
  with names \texttt{profiles-T1AW-LI-R1.csv}, manual derivatives
  
  \item
  corresponding signals filtered from the profiles in 1 CSV file,
  with name \texttt{wire-signatures-400-40.csv}, process derivatives
  
  \item
  pictures of pairwise aligned signals from same sources,
  with name \texttt{T1AW-LI-signal-aligned.png}, process derivatives
  
  \item
  cross-correlation function (CCF) values of all pairwise aligned signals,
  with name \texttt{wire\_pairwise\_ccf.csv}, process derivatives
...
introduce in details in \nameref*{sec: cutting-wires}
\tom{(cross-reference not working)} how we cut the wire and collect the
120 scans with 5 tools on 3 locations, in
\nameref*{sec: extract-profiles} how we extract profiles from the scans,
in \nameref*{sec: filtered-signals} how we filter signals from the
profiles, in \nameref*{sec: align-signals} how we align signals from
different scans, and optimize the alignment with the cross-correlation
function (CCF) values. Then, a technical validation was conducted to
further compare signals from different sources also match our
assumption. We hope this pipeline developed using this data set can be
...
\ref{fig: cut-tent-scan}(b) \tom{(need a different tent figure)}, and
the CD sides are similar from the other side of the cut, with the back
of A being C and the back of B being D. Both AB and CD sides form tent
...
\tom{@clevelandLocalRegressionModels1992 \cite{clevelandLocalRegressionModels1992}}
...
on. That is comparing each row in Figure \ref{fig: scans_pair}. We know
...
column of Figure \ref{fig: signals_pair_alignment}, which will give
...
\ref{fig: signals_pair_alignment} fulfill our expectations.
...
Figure \ref{fig: ccf_boxplot}. We can see that the CCF values for the
...
mentioned in Section \ref{sec: cutting-wires}
\tom{cross-ref not working} with more pictures for each step into a
