...
\includegraphics[width=\textwidth,height=0.15\textheight]{images/kaiweet-label-062524.JPG}

}

\subcaption{\label{fig-kaiweet-label}}
...
\caption{\label{tbl-code-overview}Overview of available codes in
``\texttt{assessment/code/}''.}

\centering{

\fontsize{12.0pt}{14.4pt}\selectfont
\begin{tabular*}{0.95\linewidth}{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}p{\dimexpr 0.477\linewidth -2\tabcolsep-1.5\arrayrulewidth}>{\raggedright\arraybackslash}p{\dimexpr 0.283\linewidth -2\tabcolsep-1.5\arrayrulewidth}>{\raggedright\arraybackslash}p{\dimexpr 0.438\linewidth -2\tabcolsep-1.5\arrayrulewidth}}
\toprule
code\_type & Description & Section \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{>{\raggedright\arraybackslash}m{0.95\linewidth}}{Process raw data} \\[2.5pt] 
\midrule\addlinespace[2.5pt]
\texttt{create\_pngs\_from\_x3p.R} & extract PNGs from raw data in \texttt{scans/} & \protect\hyperref[sec-cutting-wires]{Cutting wires} \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{>{\raggedright\arraybackslash}m{0.95\linewidth}}{Extract manual derivatives} \\[2.5pt] 
\midrule\addlinespace[2.5pt]
\texttt{create\_profiles\_from\_x3p.R} & manually extract profiles for each scan & \protect\hyperref[sec-extract-profiles]{Extract profiles} \\ 
\texttt{create-single-profile-file.R} & extract profiles as numeric values & \protect\hyperref[sec-extract-profiles]{Extract profiles} \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{>{\raggedright\arraybackslash}m{0.95\linewidth}}{Obtain computational derivatives} \\[2.5pt] 
\midrule\addlinespace[2.5pt]
\texttt{create\_signatures\_from\_profiles.R} & extract signals from profiles as numeric values & \protect\hyperref[sec-filtered-signals]{Filtered signals} \\ 
\texttt{create-images.R} & create images & \protect\hyperref[sec-filtered-signals]{Filtered signals} \\ 
\texttt{align-pairwise.R} & pairwise alignment & \protect\hyperref[sec-align-signals]{Align signals} \\ 
\texttt{all-comparison-results.R} & compare and analyze & \protect\hyperref[sec-align-signals]{Align signals} \\ 
\bottomrule
\end{tabular*}
