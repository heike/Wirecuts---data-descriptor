...
\tom{...literature review and motivation...}
Here,
we provide a data set of \tom{a table here gt package (https://posit.co/blog/great-looking-tables-gt-0-2/), 3 columns: folder name, explaination, link to section}
\begin{enumerate}
  \item
...
we introduce in details in \nameref*{sec: cutting-wires} \tom{(cross-reference not working)} how we cut the wire and collect the 120 scans with 5 tools on 3 locations,
in \nameref*{sec: extract-profiles} how we extract profiles from the scans,
in \nameref*{sec: filtered-signals} how we filter signals from the profiles,
in \nameref*{sec: align-signals} how we align signals from different scans,
and optimize the alignment with the cross-correlation function (CCF) values.
Then,
...
\tom{(need a different tent figure)},
