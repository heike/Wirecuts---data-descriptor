...
In James Genrich’s case \textcolor{red}{(how to cite?)},
...
model KWS-105,
as shown in Figure \ref{fig: kaiweet-label-062524},
...
as shown in Figure \ref{fig: T1AW-LI-R2-4edges},
...
including standard scanning protocols for the confocal microscope in Figure \ref{fig: wire-microscope-062524},
can be found in the
...
The comparison is done by manually extracting signals from scans, aligning signals,
and computing the cross-correlation function (CCF) values.

In particular,
we want to extract representative signals from each scan and use them for comparison later.
...
so we draw the line across the striations of the scan,
which are formed by ups and downs of grooves,
as shown in gray in Figure \ref{fig: T1AW-LI-R1-profiles}.
With the profile extracted,
we can then remove the overall trend,
as it can overwhelm the signals.
The signals are then extracted using locally estimated scatterplot smoothing (LOESS), and the extreme tail values are removed.
