...
Each record links a scan's unique identifier and relative file path to its tool, cutting edge, blade location, replicate, and dimensions in pixels.
...
The four variables are described in @tbl-profile-structure in the same order as the CSV columns.
Each file is named \texttt{profiles-} followed by the scan identifier and the \texttt{.csv} extension, for example, \texttt{profiles-T1AW-LI-R1.csv}.
...
::: {#tbl-signal-structure tbl-cap="Structure of the `wire-signals.csv` file." tbl-pos="hbt"}
...
The \texttt{meta.csv} file links each scan to its tool, edge, blade location, and replicate identifiers and records its dimensions in pixels.
Resolve \texttt{x3p\_path} relative to the data repository root; \texttt{id} matches the scan filename without the \texttt{.x3p} extension.
...
The \texttt{id} column in each profile CSV links it to the corresponding record in \texttt{meta.csv}.
