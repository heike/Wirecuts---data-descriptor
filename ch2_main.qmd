...
gt_table <- tibble(
  group_type = c(rep("Raw data", 2), "Manual derivatives", rep("Computational derivatives in the folder 'data-derived/'", 2), rep("Image files", 2), "Technical validation"),
  data_type = c("`scans/`", "`meta.csv`", "`profiles/`", "`wire-signals.csv.zip`", "`wire_pairwise_ccf.csv`", "`pngs/`", "`profile-images/`", "`variability-`\n`assessment/`"),
  Description = c("folder containing 120 topographic 3D scans corresponding to 30 aluminum wire cuts (`x3p` format)", "metadata for each scan: identifier, file path, tool, edge, location, replicate, and pixel dimensions (CSV format)", "folder containing one manually extracted profile per scan (CSV format)", "profile values and derived signals for all 120 scans (CSV format, in `wire-signals.csv.zip`)", "CCF values for all pairwise aligned signals (CSV format)", "folder containing images of 3D wire-cut scans (PNG format)", "folder containing images of profiles extracted from wire cuts (PNG format)", "folder containing technical-validation documents, pending an update to the current naming conventions"),
  Section = c("Scan acquisition", "Scan acquisition", "Signal extraction", "Signal extraction", "Pairwise alignment of signals", "Scan acquisition", "Signal extraction", "Technical Validation")
) %>%
  group_by(group_type) %>%
...
The \texttt{meta.csv} file contains one record for each of the 120 scans, with eight variables described in @tbl-meta-structure.
...
The \texttt{profiles/} folder contains 120 CSV files, one manually extracted profile per scan, with one sampled point per row.
...
The \texttt{data-derived/wire-signals.csv.zip} archive contains \texttt{data-derived/wire-signals.csv}, with one sampled profile point per row for all 120 scans.
Its 12 variables are described in @tbl-signal-structure in CSV column order.
...
The \texttt{data-derived/wire\_pairwise\_ccf.csv} file contains one row for each ordered pair of the 120 signals, for 14,400 rows in total.
Its 19 variables are described in @tbl-pairwise-structure in the same order as the file columns.
Prefixes \texttt{ID1\_} and \texttt{ID2\_} identify the first and second signal in each comparison.
...
@fig-ccf-boxplot groups CCF distributions by different edge, different location, different tool, and same source, with the paired edges identified on the horizontal axis.
@fig-ccf-ROC shows classification performance separately for long and short edge classes.
...
string <- tibble(
  group_type = c("Inspect raw scans", rep("Extract profiles", 2), "Derive signals", rep("Compare signals", 3)),
  code_type = paste0("`", code_files, "`"),
  Description = c("obtain images of `x3p` files in `scans/`", "manually extract profiles from each scan", "combine profile information", "derive signals from each profile", "create images for pairwise alignment", "compute pairwise alignment CCF values", "visualize comparison results"),
  Section = c("Scan acquisition", "Signal extraction", "Signal extraction", "Signal extraction", "Pairwise alignment of signals", "Pairwise alignment of signals", "Data Overview and Technical Validation")
) %>%
