...
affiliations: 
  ...
  - id: 5
    ...
    country: USA
abstract: |
 Wire-cut striation marks can provide evidence for forensic source identification, but publicly available datasets and reproducible quantitative workflows for analyzing these marks are limited.
...
The cutting tools were five Kaiweets KWS-105 wire cutters.
The material was general-purpose, 16-gauge, 1.5-mm anodized pure aluminum wire sold for crafting.
...
The physical properties of aluminum wire make it suitable for retaining marks while remaining relatively easy to bend and nontoxic.
...
This hardness allowed the tools to leave marks while helping the cut samples resist damage during handling under the microscope.
For comparison, the hardness values of pure lead and copper are 5 HB and 35 HB, respectively.
...
For each of the five wire cutters, we made two replicate cuts at each of three blade locations (inner, middle, and outer), as shown in @fig-cut-tent-scan (a).
...
A 4-inch wire segment was used for each replicate and divided into two 2-inch sides, AB and CD.
Each side was separated along the bend into two pieces, yielding four cut surfaces (A, B, C, D).
As shown in @fig-cut-tent-scan (c), C is the reverse of A and D is the reverse of B.
...
(c) Four cut-surface samples labeled A, B, C, and D after separation.
...
The filename components are T(ool) 1/2/3/4/5, (Edge) A/B/C/D, W(ire), L(ocation) I(nner)/M(iddle)/O(uter), and R(eplicate) 1/2.
For example, T1AW-LI-R1 denotes the first replicate of a piece cut by tool 1 on edge A at the inner location.
...
Numerical comparisons between two replicates cannot be performed directly on the \texttt{x3p} files.
We must first extract representative profiles from the scans.
A profile is a sequence of surface-height values along a user-drawn line.
After processing, this profile becomes a signal representing one scan and is used for subsequent comparisons.
...
The resulting smoothed signal was used for subsequent comparisons, as shown in @fig-T1AW-LI-R1-profiles-signals (c).
...
As an example of a different-source comparison, we selected T1AW-LI-R1 and T2AW-LI-R1, shown in @fig-T1AW-LI-R1-T2AW-LI-R1-alignment (a) and (b).
...
We also summarized the resulting CCFs for all pairwise comparisons in boxplots and receiver operating characteristic (ROC) curves, shown in @fig-ccf-boxplot and @fig-ccf-ROC.
...
Replicate variability was assessed using two different cuts at the same edge and position, as illustrated by the comparison in @fig-signals-pair-alignment in Align signals.
...
For edge A at staging 3, we performed four acquisitions and omitted the A1 designation from the first acquisition label, so R2-S3 is equivalent to R2-S3-A1.
For the other three edges, we conducted two acquisitions at staging 3, denoted by R2-S3 and R2-S3-A2.
...
Consequently, variability should decrease along this sequence.
...
We visualized the results as scatter plots in @fig-rep_staging_acquisition, which also shows intervals extending two standard deviations above and below the mean.
...
Dashes indicate that the standard deviation could not be calculated because only one CCF value was available."}
...
Users who wish to inspect or process these files in R can use the \texttt{x3ptools} package and its reference manual, available from CRAN [@hofmannX3ptoolsToolsWorking2018].
The \texttt{meta.csv} file links each scan to its tool, edge, blade location, and replicate identifiers.
Users who do not wish to repeat manual profile extraction can begin with the supplied profiles and processed signals.
...
Custom R code used to inspect raw scans, extract profiles, derive signals, align signals, and generate the validation outputs is available in the \texttt{assessment/code} folder at \url{https://github.com/heike/wirecuts-data}.
