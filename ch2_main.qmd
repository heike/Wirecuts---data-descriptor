...
authors: 
  ...
  - name: Jeff Salyards
    ...
    email: jdsal@msn.com
    corresponding: false
  - name: \tom{order? Jeff the last? check details? affiliations?}
    corresponding: false  
affiliations: 
  - id: 1
  ...
  - id: 4
    name: Iowa State University
    department: Department of Genetics, Development and Cell Biology (GDCB)
    city: Ames
    state: IA
  - id: 5
    name: Iowa State University
...
Although the Association of Firearm and Toolmark Examiners (AFTE) developed the Theory of Identification [@afte], which outlines the process of comparing tool marks,
...
Early research by Ma et al. [@maNISTBulletSignature2004] and Zheng et al. [@zhengNISTBallisticsToolmark2016] has focused on collecting and distributing datasets for this purpose and providing a foundation for future advancements in tool mark analysis.
...
string <- tibble(
  group_type = c(rep("Raw data", 2), rep("Manual derivatives", 1), rep("Computational derivatives", 2), rep("Image files")),
  data_type = c("`scans/`", "`meta.csv`", "`profiles`", "`signals`", "CCF values", "`aligned-signals`"),
  Description = c("folder containing 120 topographic 3d scans of aluminum wire cuts in x3p format", "meta information for each cut with tool, blade, and location information (csv format)", "folder of files with manually extracted profiles (csv format)", "folder of signals, processed from corresponding profile (csv format)", "CCF values of all pairwise aligned signals in 1 CSV", "pictures of pairwise aligned signals from same sources in PNGs"),
  Section = c("[Cutting wires](#sec-cutting-wires)", "[Cutting wires](#sec-cutting-wires)", "[Extract profiles](#sec-extract-profiles)", "[Filtered signals](#sec-filtered-signals)", "[Align signals](#sec-align-signals)", "[Align signals](#sec-align-signals)")
) %>%
  gt(rowname_col = "data_type", groupname_col = "group_type") %>%
  tab_options(table.width = pct(95)) %>%
  cols_width(Description ~ pct(45), everything() ~ pct(25)) %>%
  fmt_markdown() %>%
  as_latex()
...
we introduce in detail in [Cutting wires](#sec-cutting-wires) \tom{hyperlink location incorrect for unnumbered sections} how we cut the wire and collect the 120 scans with 5 tools on 3 locations,
...
![](images/kaiweet-label-062524.JPG){#fig-kaiweet-label height=15%}
...
\(a) A Kaiweets wire cutter of model KWS-105 was used to cut the wire. \tom{we don't have another high resolution pic showing wirecutter cutting wire} (b) A confocal microscope was used to scan the wire surfaces. \tom{need an extra pic of the tip} (c) After separating 2 tent structures by the connecting position, we obtained 4 samples - 2 samples from blade A and B, and others from blade C and D. \tom{full requirements see \href{https://www.nature.com/sdata/publish/submission-guidelines\#figures}{https://www.nature.com/sdata/publish/submission-guidelines\#figures}}
...
The profile function is along the line is plotted in @fig-T1AW-LI-R1-profiles-plot.
...
Cleveland et al. [@clevelandLocalRegressionModels1992]\tom{???}
...
and we maximize the cross-correlation function (CCF) values between the signals to numerically find the best alignment.
For example,
...
For the validation of the scans and their processing we investigate the correlation scores of pair-wise aligned signals. Large scores between signals are indicative of being made by the same tool. As showed previously in Figure \ref{fig-signals-pair-alignment} in [Align signals](#sec-align-signals), we would expect a high correlation score between signals from scans of wires cut with the same tool. For signals from scans of wires cut with a different tool, we would expect a low correlation score. For example, we have two scans from different tools, T1AW-LI-R1 and T2AW-LI-R1, as shown in @fig-T1AW-LI-R1 and @fig-T2AW-LI-R1. The alignment is shown in @fig-T1AW-LI-R1-T2AW-LI-R1 with 0.2 CCF value, which is low, as expected.
...
#| tbl-cap: Overview of available codes in "`assessment/code/`".
#| results: asis

string <- tibble(
  group_type = c(rep("Process raw data", 1), rep("Extract manual derivatives", 2), rep("Obtain computational derivatives", 4)),
  code_type = c("`create_pngs_from_x3p.R`", "`create_profiles_from_x3p.R`", "`create-single-profile-file.R`", "`create_signatures_from_profiles.R`", "`create-images.R`", "`align-pairwise.R`", "`all-comparison-results.R`"),
  Description = c("extract PNGs from raw data in `scans/`", "manually extract profiles for each scan", "extract profiles as numeric values", "extract signals from profiles as numeric values", "create images", "pairwise alignment", "compare and analyze"),
  Section = c("[Cutting wires](#sec-cutting-wires)", "[Extract profiles](#sec-extract-profiles)", "[Extract profiles](#sec-extract-profiles)", "[Filtered signals](#sec-filtered-signals)", "[Filtered signals](#sec-filtered-signals)", "[Align signals](#sec-align-signals)", "[Align signals](#sec-align-signals)")
) %>%
  gt(rowname_col = "data_type", groupname_col = "group_type") %>%
  tab_options(table.width = pct(95)) %>%
  cols_width(Description ~ pct(45), Section ~ pct(30), everything() ~ pct(50)) %>%
  fmt_markdown() %>%
  as_latex()
