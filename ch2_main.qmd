...
\hh{Intro sentence needs to describe an action that forensic experts do}

<!-- Source Problem-->
An important part of a forensic analysis is the investigation of marks left at a crime scene. Forensic examiners are in particular interested in the origin of those marks, ie. in the investigation of their source.  This is known as the Source Identification Problem in Forensic Science \hh{citation?}. We distinguish between a specific source problem, where the examiner is interested in whether a mark was left by a specific tool, and the common source problem, where the focus of the problem is on whether two marks were left by the same source. 

In this paper we are interested in the second of these problems: in particular, we want to determine whether two marks on cut wires were made by the same tool. 

When a bladed tool cuts a wire, striation marks are left on the cut surface of the wire. \hh{picture?}
...
```{=latex}
\begin{figure}[hbt]
\includegraphics[width='50%']{"images/IMG_72116.jpg"}
\caption{\label{fig-cut-microscope}Microscopic close-up of striations left by a blade on the cut end of a wire.}
\end{figure}
```
...
Reports by the National Research Council [@nas2009] and the President's Council of Advisors on Science and Technology [@pcast] criticized this lack of objectivity and called for more automatic and reproducible methods to analyze tool marks that also allow an assessment of error rates in the process.
...
string <- tibble(
  group_type = c(rep("Raw data", 2), rep("Manual derivatives", 1), rep("Computational derivatives", 2), rep("Image files", 3)),
  data_type = c("`scans/`", "`meta.csv`", "`profiles/`", "`data-derived/wire-signals.csv.zip`", "`data-derived/\nwire_pairwise_ccf.csv.zip`", "`pngs/`", "`profile-images/`", "`assessment/analysis-manual/\nprocessing-wires.html`"),
  Description = c("folder containing 120 topographic 3d scans of aluminum wire cuts (x3p format)", "meta information for each cut with tool, blade, and location information (CSV format)", "folder of files with manually extracted profiles (CSV format)", "signals processed from corresponding profile (CSV format)", "CCF values of all pairwise aligned signals (CSV format)", "folder containing pictures of 3d scans of wire cuts (PNG format)", "folder containing pictures of profile extracted from wire cuts (PNG format)", "display of pairwise aligned signals from the same sources (HTML format)"),
  ...
  cols_width(data_type ~ pct(55), everything() ~ pct(25)) %>%
  fmt_markdown() %>%
  as_latex()
...
we introduce in detail in [Cutting Wires](#sec-cutting-wires) \tom{hyperlink location incorrect for unnumbered sections} how we cut the wire and collect the 120 scans with 5 tools,
