...
contributions: |
 Let's follow the Elsevier definitions: https://www.elsevier.com/researcher/author/policies-and-guidelines/credit-author-statement
...
Although the Association of Firearm and Toolmark Examiners (AFTE) developed the Theory of Identification  \tom{incorrect citation format} [@afte], which outlines the process of comparing tool marks,
...
string <- tibble(
  ...
  data_type = c("`x3p-files`", "`meta.csv`", "`profiles`", "`signals`",  "CCF values", "`aligned-signals`"),
...
crossrefs <- str_extract_all(pattern="@[^ {]*", string[1])[[1]]
...
# not sure why the command needs to be escaped twice ... but it works
cross_latex <- sprintf("\\\\autoref{%s}", gsub("@", "", crossrefs)) 

string2 <- string[1]
for (i in seq_along(crossrefs)) {
    string2 <- str_replace_all(string2, crossrefs[i], cross_latex[i])
}
string[1] <- string2
