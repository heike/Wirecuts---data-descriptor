...
abstract: |
 ...
 We present a dataset of 134 three-dimensional topographic scans in \texttt{x3p} format from aluminum wires cut with five KWS-105 wire cutters.
 The main experiment contains 120 scans: two replicates at each of three cutting locations for each of the 20 tool-edge combinations.
 Fourteen additional scans assess staging and acquisition variability for a randomly selected tool and location.
...
knitr::include_graphics("images/kaiweet-sketch.png")
```
...
These repeated scans do not represent additional cuts; their acquisition design is described in Technical Validation later.
...
The \texttt{data-derived/wire\_pairwise\_ccf.csv} file contains all 17,956 ordered pairs of the 134 signals, including self-comparisons and both orders of each distinct pair.
The main-experiment subset contains 14,400 ordered pairs of the 120 signals at staging 1 and acquisition 1.
@tbl-pairwise-structure describes the file's 12 variables in CSV column order.

::: {#tbl-pairwise-structure tbl-cap="Structure of the `wire_pairwise_ccf.csv` file." tbl-pos="hbt"}
...
As descriptive summaries of the 120 main-experiment scans at staging 1 and acquisition 1, @fig-ccf-tilemap displays pairwise CCF values organized by tool, cut surface, blade location, and replicate.
The diagonal running from the lower left to the upper right contains self-comparisons, whereas the off-diagonal cells compare distinct signals.
...
#|   The matrix contains the 14,400 ordered comparisons of the 120 main-experiment scans at staging 1 and acquisition 1.
#|   Rows and columns group scans by tool, cut surface, blade location, and replicate.
#|   Orange cells indicate larger CCF values and gray cells indicate smaller values; the inset schematically shows the location and replicate arrangement within the boxed edge-to-edge comparison block, without encoding CCF values.
#| fig-pos: hbt
...
@fig-ccf-boxplot groups CCF distributions from left to right by same source, different location, different edge, and different tool, in order of decreasing expected similarity, with the paired edges identified on the horizontal axis.
...
@fig-ccf-ROC shows classification performance separately for long and short edge classes, comparing same-source pairs with different-tool pairs.
...
#|   Comparisons use the 120 main-experiment scans at staging 1 and acquisition 1.
#|   Facets are ordered from left to right by decreasing expected similarity: same source, different location, different edge, and different tool.
...
#|   Comparisons use the 120 main-experiment scans at staging 1 and acquisition 1.
#|   Curves compare same-source pairs with different-tool pairs separately for long and short edge classes; same-tool different-source pairs are excluded.
...
The 22 scans from tool 2 at the middle location were selected from the shared files for this assessment: eight main-experiment scans and 14 additional scans of replicate 2.
...
To assess staging variability, replicate 2 was rescanned under the confocal microscope; the staging label S identifies these repeated stagings in the naming convention.
...
Their pairwise CCFs were summarized by the mean and standard deviation (SD) using the comparison categories defined below.
...
Replicate comparisons pair the scan of replicate 1 with every scan of replicate 2, including its repeated stagings and acquisitions.
Staging comparisons pair scans of replicate 2 from different stagings, including all acquisitions within those stagings.
Acquisition comparisons pair scans from the same replicate and staging but different acquisitions.
These categories contain, respectively, 6, 9, and 6 comparisons for edge A and 4, 5, and 1 for each of edges B, C, and D.
...
@fig-rep_staging_acquisition shows the scatter plots and intervals extending two standard errors (SEs) above and below the mean.
For each comparison category and edge, the plotted SE is calculated as $\mathrm{SD}/\sqrt{n}$, where $n$ is the number of distinct pairwise CCF values.
Higher orange mean lines indicate an increase in the mean CCF; interval widths depend on both the SD and the number of comparisons.
Because comparisons share scans, these intervals are descriptive summaries rather than confidence intervals based on independent observations.
...
Mean CCF increases from replicate to staging to acquisition comparisons for every edge, and the acquisition-to-staging mean differences are smaller than the staging-to-replicate differences.
The SDs do not decrease uniformly: edge B has a higher staging SD than replicate SD, whereas edges A, C, and D have lower staging SDs.
For acquisition comparisons, SD is defined only for edge A, which has six pairs; the single pair for each other edge does not permit an estimate of dispersion.
...
#|   Panels A, B, C, and D correspond to the four cut surfaces of tool 2 at the middle location, using 22 scans from the shared dataset.
#|   Gray points show pairwise CCF values, orange horizontal lines show means, and shaded intervals extend two standard errors above and below each mean, with SE calculated as SD divided by the square root of the number of comparisons.
#|   Each distinct unordered pair is counted once within the replicate, staging, or acquisition category; no interval is shown for a category with only one pair.
...
For variability assessment, select tool 2 at the middle location and use the hierarchical comparison flags to distinguish replicate, staging, and acquisition comparisons as described in Technical Validation.
...
The code is available in the root-level \texttt{code/} folder at \url{https://github.com/heike/wirecuts-data}.
...
string <- tibble(
  ...
  Section = c("Scan acquisition", "Signal extraction", "Signal extraction", "Signal extraction", "Pairwise alignment of signals", "Data Overview and Technical Validation", "Technical Validation")
) %>%
...
The provided code implements the processing, comparison, validation, and visualization workflow described in Data Acquisition and Processing and Technical Validation.
...
The \texttt{renv/} folder and project startup file support restoration of this environment.
