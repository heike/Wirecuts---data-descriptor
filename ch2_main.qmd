...
In the second step, each scan's signal was extracted by removing trends and outliers from the profile's surface values using two LOWESS filters with bandwidths of 400 $\mu m$ and 40 $\mu m$, respectively [@clevelandLocalRegressionModels1992].
...
Using `x3ptools::x3p_extract_profile`, one line was manually drawn across each scan, perpendicular to the dominant striation direction, and the surface-height values and extraction coordinates were exported.

One profile for each scan is included as a CSV file in the repository using the same naming convention as the scans, with the prefix \texttt{profiles-}.
...
After signals were extracted from different scans, all possible signal pairs were compared.
For each pair, the CCF was calculated across relative shifts, and the shift with the maximum CCF was retained as the optimal alignment.
