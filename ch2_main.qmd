...
@fig-signals-pair-alignment shows all signals corresponding to cuts 1 and 2 with tool 1 closest to the pivot area (inner location). 
Each of the four edges correspond to one row in the figure. The lines in the first two columns correspond to the extracted signals for cuts 1 and 2, respectively. The third column shows the two signals aligned in the location of the maximal CCF. The cross-correlation value for each pair is included as part of the title of the chart.

The code to align signals pairwise is available in
`code/6-align-pairwise.R` and `data-derived/wire_pairwise_ccf.csv` is the file with the CCF similarity scores of all pairwise aligned signals.
