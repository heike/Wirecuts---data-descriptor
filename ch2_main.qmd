...
The two roof sides of each cut surface were then scanned separately, yielding scans A and B from cut side AB and scans C and D from cut side CD.
...
In the second step, we extracted each scan's signal by removing trends and outliers from the profile's surface values using two LOWESS filters with bandwidths of 400 $\mu m$ and 40 $\mu m$, respectively [@clevelandLocalRegressionModels1992].
...
To avoid potential artificial ``hooks'' at either end of a signal, we used two-sample $t$-tests on the surface values in the outermost 2.5\% intervals along the horizontal axis.
Specifically, we calculated the summary statistics (sample size, mean, and standard deviation) of the signal values within the intervals $[0, 2.5)\%$, $[2.5, 5)\%$, $(95, 97.5]\%$, and $(97.5, 100]\%$.
At each end, the test compared the outermost 2.5\% interval with the adjacent interval.
We retained the signal values in the outermost interval only when the corresponding $p$-value was above 0.01, indicating no significant shift.

The widths of these intervals correspond to the choice of bandwidths and the length of the signal.
We determined the interval widths using cross-validation.
