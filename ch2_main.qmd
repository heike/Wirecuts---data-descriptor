...
Acquisition is included only when staging is present.
...
The \texttt{variability-assessment/} folder follows the same folder organization and data-file structure as the main repository for the additional scans collected under different staging and acquisition conditions.
...
The \texttt{data-derived/wire-signals.csv.zip} archive contains \texttt{data-derived/wire-signals.csv}, containing signals for all 120 scans.
@tbl-signal-structure describes its 12 variables in CSV column order.
...
The \texttt{data-derived/wire\_pairwise\_ccf.csv} file contains all 14,400 ordered pairs of the 120 signals.
@tbl-pairwise-structure describes its 9 variables in CSV column order.
...
string <- tibble(
  group_type = c("Inspect raw scans", rep("Extract profiles", 2), "Derive signals", rep("Compare signals", 2)),
  code_type = paste0("`", code_files, "`"),
  Description = c("obtain images of `x3p` files in `scans/`", "manually extract profiles from each scan", "combine profile information", "derive signals from each profile", "compute pairwise alignment CCF values", "visualize comparison results"),
  Section = c("Scan acquisition", "Signal extraction", "Signal extraction", "Signal extraction", "Pairwise alignment of signals", "Data Overview and Technical Validation")
) %>%
